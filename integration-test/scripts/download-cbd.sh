...
docker run $rm_flag --user="0" --entrypoint init.sh -v cbd-source:/var/workspace docker-private.infra.cloudera.com/cloudera_thirdparty/jpco/git:1.0 https://github.com/hortonworks/cloudbreak-deployer.git CB-9112-add-infrastructure-mock cloudbreak-deployer

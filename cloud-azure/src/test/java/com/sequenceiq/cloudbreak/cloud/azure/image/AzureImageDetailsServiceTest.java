...
public class AzureImageDetailsServiceTest {
    ...
    @InjectMocks
    private AzureImageDetailService underTest;
    ...
    public void testGetImageDetails() {
        ...
        when(customVMImageNameProvider.get(REGION_NAME, FROM_VHD_URI)).thenReturn(CUSTOM_IMAGE_NAME);

        AzureImageDetails azureImageDetails = underTest.getImageDetails(RESOURCE_GROUP_NAME, FROM_VHD_URI, authenticatedContext, azureClient);

        assertEquals(REGION_NAME, azureImageDetails.getRegion());
        assertEquals(CUSTOM_IMAGE_ID, azureImageDetails.getImageId());
        assertEquals(CUSTOM_IMAGE_NAME, azureImageDetails.getImageName());
        assertEquals(RESOURCE_GROUP_NAME, azureImageDetails.getResourceGroup());
        verify(azureResourceIdProviderService).generateImageId(SUBSCRIPTION_ID, RESOURCE_GROUP_NAME, CUSTOM_IMAGE_NAME);
        verify(customVMImageNameProvider).get(REGION_NAME, FROM_VHD_URI);
    }
